...
int main(void)
{
    ...
    while(op)
    {
        ...
        case 23:
            if(CreatAnotherBiTree(&T, BiTree_L) == OK) printf("创建新的二叉树成功！\n");
            else printf("操作失败！\n");
            system("pause");
            break;
...
status DeleteChild(BiTree T, Node *p, int LR, BiTreeList BiTree_L)
{
    if(LR == 0)
    {
        DestroyBiTree(&(p->LeftChild), BiTree_L);    //销毁左子树
        return OK;
    }
    else if(LR == 1)
    {
        DestroyBiTree(&(p->RightChild), BiTree_L);    //销毁右子树
        return OK;
    }
