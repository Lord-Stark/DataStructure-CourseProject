...
int main(void)
{
    ...
    while(op)							//case 0:op=0（循环出口）
    {
        ...
        if(scanf("%d", &op) != 1)
        {
            fflush(stdin);    //若输入数字之外的字符，清空标准输入流里的数据，并将op置为default
            op = 18;
        }
        switch(op)
        {
        case 1:
            if(IntiaList(&L, Head_L) == OK) printf("线性表创建成功！\n");
            else printf("线性表创建失败！\n");
            system("pause");
            break;
...
status PriorElem(Linklist L, ElemType cur, ElemType *pre_e)
{
    ...
    {
        *pre_e = pre_q->data;    //若找到了值为的cur的数据元素，且该数据元素的前驱有定义，取出其前驱的值
        return OK;
    }
...
status NextElem(Linklist L, ElemType cur, ElemType *next_e)
{
    ...
    {
        *next_e = p->next->data;    //若找到了值为的cur的数据元素，且该数据元素的后继有定义，取出其后继的值
        return OK;
    }
...
status CreatList(Linklist L)
{
    ...
    while(scanf("%d", &n) != EOF)
    {
        ...
        p->data = n;							//录入数据元素数据
        pre_p->next = p;						//延伸链表
        pre_p = p;							//pre_p向后移动，记录当前链表的最后一个节点，也就是即将生成的新节点的前一个结点
        L->data++;							//数据元素个数加一
    }
    if(p != NULL) p->next = NULL;
    return OK;
...
status LoadList(Linklist *L, HeadNodeList Head_L)
{
    ...
    (*L) = NULL;								//清空现有头指针的指向
    if(IntiaList(L, Head_L) == ERROR) exit(OVERFLOW);					//初始化线性表
    pre_p = *L;
